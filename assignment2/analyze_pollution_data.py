...
from analytic_tools.utilities import get_diagnostics, display_diagnostics, is_gas_csv, get_dest_dir_from_csv_file, merge_parent_and_basename
...
def restructure_pollution_data(pollution_dir: str | Path, dest_dir: str | Path) -> None:
    ...
    if not pollution_dir.is_dir() or not dest_dir.is_dir():
            raise NotADirectoryError(" Both pollution_dir and dest_dir must point to exsiting directories.")
    ...
    for path in contents:
        ...
        if path.suffix == ".csv" and is_gas_csv(path):
            ...
            dest_file_path = dest_gas_dir / new_name
            copy(path, dest_file_path)
...
def analyze_pollution_data(work_dir: str | Path) -> None:
    ...
    if not work_dir.is_dir():
        raise NotADirectoryError("This is not a directory, check again, and try again please!")
    ...
    dictionary = get_diagnostics(work_dir / "pollution_data")

    display_diagnostics(work_dir / "pollution_data", dictionary)
